  ...
  return <section className="service-section" id="services">
    ...
    <div className="service-explorer" ref={ref}>
      ...
      <div className="service-visual">
        <AnimatePresence mode="wait"><motion.div className="service-image" key={active} initial={enabled ? { opacity: 0, scale: 1.04 } : false} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: enabled ? 0 : 1 }} transition={{ duration: enabled ? .3 : 0 }}><Image src={services[active].image} alt={services[active].imageAlt} fill sizes="(max-width: 900px) 100vw, 50vw" style={{ objectPosition: services[active].position }} /></motion.div></AnimatePresence>
