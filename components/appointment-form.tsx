...
export function AppointmentForm() {
  ...
  useEffect(() => {
    ...
    const select = ((event: CustomEvent<string>) => {
      if (serviceNames.includes(event.detail)) {
  ...
  const select = (name: "service" | "vehicle" | "time", label: string, values: string[]) => <FormSelect name={name} label={label} value={data[name]} options={values} onChange={value => update(name, value)} error={errors[name]} />;
  const variants = {
